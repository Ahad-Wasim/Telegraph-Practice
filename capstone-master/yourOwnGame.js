...
var allRows = _.each(gameBoard,function(val){
  console.log(val);
}); 
...
_.each(gameBoard,function(val){
   _.each(val,function(nextVal){
    console.log('this is the inner loop',nextVal);
  });
}); 
...
_.each(gameBoard[0],function(firstRow){
   console.log('This is the certain position',firstRow);
});
...
_.each(gameBoard[0],function(eachBox){
  eachBox.color = 'red';
}); 
  ...
  // Now let's do the same thing using map on the second row of our gameBoard.
    ...
    // Let's pseudocode!
     ...
     //  var mapColors = _.map(colors,function(value){
     //      return value;
     // }); 

     //  console.log(mapColors);

      // NOTE ALWAYS STORE MAP IN A VARIALBE 
      ...
      // On each iteration of map
        ...
        var changeColors = _.map(colors,function(val,index){
          return  {
                      position:[1,index],
                      color:val,
                      gamePiece:'',
  ...
  // Now that we've figured out how to use map and each to change the colors in a row, let's nest them inside of another each to change all of the rows!
    ...
    // Let's warm up to this by doing it the way we would have before we knew how to program functionally: using nested for loops. 
      ...
      //   var gameBoardRows = gameBoard[i];
      //   console.log('This is gameBoard rows',gameBoardRows);
      //   for(var j = 0;j<gameBoardRows.length;j++){
      //     console.log('This is the inner loop', gameBoardRows[j]);
      //   }
      // }
    ...
    // Great! Now that we've changed the color of each square to orange using for loops, let's transition this over to functional programming.
      ...
      // First, replace the inner for loop with an each statement that changes the color of each square to blue. Be sure to write a new each statement for this- don't just copy and paste the one you've written up above. We want you to get as much practice typing these out as possible!
       ...
       // for(var i =0;i<gameBoard.length;i++){
       //    var gameBoardRows = gameBoard[i];   
       //    _.each(gameBoardRows,function(value,index){
       //        gameBoardRows[index].color = 'blue';
       //    });
       //   }
    ...
    var purple = ['purple','purple','purple','purple','purple','purple','purple','purple'];

      // Let's replace our inner each loop with map, changing the colors of all the squares to purple this time.

      //  _.each(gameBoard,function(row){
      //      var map =  _.map(purple,function(colorPurple,key){
      //       return
      //             {
      //                position:[row, key],
      //                color:colorPurple,
      ...
      // }); 
            ...
            _.each(gameBoard,function(row, rowIndex){
               var colorsResult = _.map(purple,function(colorPurple,key){
               ...
               console.log('This is colorsResult',colorsResult);
              gameBoard[rowIndex] = colorsResult;
            }); // closes each
...
// Now that we have a decent understanding of the gameBoard. Now let's test out the makePiece function. 
  ...
  var returnedDino = _.filter(gameBoard[3],function(squareBox){
                              return squareBox.gamePiece !== ''; 
                      });

  // console.log(returnedDino);

  //arif.furqon@gmail.com

  // Now try adding gamePieces to a couple of different rows throughout the board using this makePiece funcion.

  makePiece(gameBoard,[0,0],'Ahad');
  gameBoard[0][0].gamePiece.imageURL = "http://coolwildlife.com/wp-content/uploads/galleries/post-531/Wolf%20Pictures%20001.jpg";
  makePiece(gameBoard,[1,5],'Haris');
  gameBoard[1][5].gamePiece.imageURL = "http://coolwildlife.com/wp-content/uploads/galleries/post-345/Wolf%20Pictures%20033.jpg";
  ...
  // Can you use each, in conjunction with filter, to find all the squares on the whole board that have a piece on them? 
    ...
    var nonEmptyGamePieces = [];

    _.each(gameBoard,function(row,rowIndex){
       var returnedPieces = _.filter(row,function(square,gamePieceIndex){
                                return square.gamePiece;
                            });
       nonEmptyGamePieces.push(returnedPieces);
    });

    console.log('This is the nonEmptyGamePieces', nonEmptyGamePieces);
...
// What reduce does is:
  ...
  // Write out your own invocation of reduce that gives you the results of multiplying all the values in testArr together. 
    ...
    var mulitply = _.reduce(testArr,function(prevValue,currentVal){
      return prevValue * currentVal;
    });

    console.log('This is multiply', mulitply);
  ...
  var reduceObject = _.reduce(codingPoints,function(prevValue,currentVal){
    return prevValue + currentVal ;
  });

  console.log('reduceObject returns this:',reduceObject);
  ...
  // reduce also takes an optional starting value that we haven't been giving it yet.
    ...
    var startingReduce = _.reduce(codingPoints,function(prevValue,currentVal){
                          return prevValue + currentVal;
                        },50);

    console.log('this is the startingReduce',startingReduce);
    ...
    var multiplyTen = _.reduce(testArr,function(prevValue,currentVal){
      return prevValue * 10;
    },10);

    console.log('This is multiplyTen:',multiplyTen);
  ...
  // Let's write out code that reduces the codingPoints object down to just an array of values that are larger than 1000. So we'd expect to get the following: [1200, 8000000]
    ...
    var filterObject = _.filter(codingPoints,function(value){
      return value > 1000;  
    });

    console.log('this is the filterObject',filterObject);
  ...
  // Now let's think through how we could use reduce to turn an array of nested arrays into a single array. 
    ...
    // use a nested loop to loop through each value inside the nested array.

   var reducedNestedArray = _.reduce(nestedArrs,function(prevValue,nextVal){
      return prevValue.concat(nextVal);
   });

   console.log(reducedNestedArray);
  ...
  // Another interesting use of reduce is with booleans.
    ...
    // var myFriends = _.filter(friends,function(currentVal){
    //    if(currentVal === true){
    //     return  currentVal;
    //   }
    // });

    // console.log('this is my friends',myFriends);
      ...
      var friendsReduce = _.reduce(friends, function(accumulated, current) {
      ...
      console.log('this is the friendsReduce',friendsReduce);
    ...
    // Now let's put this to use another way. 
      ...
      // Let's make this poor bouncer's job a bit easier by reducing this collection down to a single true or false value of whether everyone in this group is at least 21 or not.
        ...
        //REDUCE ALWAYS RETURN ONE VALUE

     var bouncersSave =  _.reduce(bouncersNightmare,function(prevValue,currentVal){
                              
                              return prevValue && currentVal >=21;
                         },true);

     console.log('This is the bouncersSave',bouncersSave);
...
// Having that information scattered throughout a bunch of different arrays seems messy. You can probably think of plenty of cases where we'd want to have all that information collected into a single array. 
  ...
  var reducePictureArrays = _.reduce(nonEmptyGamePieces,function(accumulated,currentVal){
      return accumulated.concat(currentVal);
  },[]);

  console.log('This is the reducePictureArrays',reducePictureArrays);
...
// 3. Now use two (nested) each loops to add these pieces to the board. Remember that we have the makePiece function! 
  ...
  _.each(gameBoard,function(row,index){
     _.each(row,function(square,squareIndex){
        // if it's row 1 or 2, add pieces for playerNames[0]
        // if it's row 7 or 8, add pieces for playerNames[1]
        makePiece(gameBoard,[index,squareIndex],piecesToAdd[squareIndex]);
      });
  });

  _.each(playerNames, function(playerName) {
    // for each player, do this:
    var piecesAddedCount = 0;
    // once that gets up to 8, move to the next line
    _.each(piecesToAdd, function(pieceName) {
  ...
  console.log('I hopefully added the index',gameBoard);
  ...
  _.each(gameBoard,function(row,rowIndex){
    _.each(row,function(row,squareIndex){
